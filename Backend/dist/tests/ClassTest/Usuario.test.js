...
describe('Usuario', () => {
    describe('Criação e Validação', () => {
        it('deve criar usuário válido', () => {
            const usuario = new Usuario_1.Usuario('1', 'João Silva', 'joao@email.com', 'hash_senha', 'Rua A, 123', '11999999999');
            expect(usuario.id).toBe('1');
            expect(usuario.nome).toBe('João Silva');
            expect(usuario.email).toBe('joao@email.com');
        });
        it('deve lançar erro com nome inválido', () => {
            expect(() => new Usuario_1.Usuario('1', '', 'email@test.com', 'hash', null, null)).toThrow('Nome invalido');
            expect(() => new Usuario_1.Usuario('1', 'A', 'email@test.com', 'hash', null, null)).toThrow('Nome invalido');
        });
        it('deve lançar erro com email inválido', () => {
            expect(() => new Usuario_1.Usuario('1', 'João', '', 'hash', null, null)).toThrow('Email inválido');
            expect(() => new Usuario_1.Usuario('1', 'João', 'invalido', 'hash', null, null)).toThrow('Email inválido');
        });
        it('deve lançar erro com senha inválida', () => {
            expect(() => new Usuario_1.Usuario('1', 'João', 'joao@email.com', '', null, null)).toThrow('Senha inválida');
            expect(() => new Usuario_1.Usuario('1', 'João', 'joao@email.com', '123', null, null)).toThrow('Senha inválida');
        });
    });
    describe('verificarSenha', () => {
        it('deve verificar senha correta', async () => {
            const usuario = new Usuario_1.Usuario('1', 'João', 'joao@email.com', 'hash_senha', null, null);
    ...
    describe('atualizarPerfil', () => {
        it('deve atualizar perfil com sucesso', () => {
            const usuario = new Usuario_1.Usuario('1', 'Nome', 'email@test.com', 'hash', 'End', '111');
    ...
    it('não deve permitir alteração do email após criação', () => {
        const usuario = new Usuario_1.Usuario('1', 'João Silva', 'joao@email.com', 'hash', null, null);
