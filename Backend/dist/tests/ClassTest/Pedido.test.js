...
describe('Pedido', () => {
    const mockItem1 = { subtotal: 100 };
    const mockItem2 = { subtotal: 50 };
    const dataAtual = new Date();
    describe('Criação e Validação', () => {
        it('deve criar pedido válido', () => {
            const pedido = new Pedido_1.Pedido('1', 'user123', [mockItem1], 'pix', 'realizado', dataAtual);
        ...
        it('deve lançar erro com usuário vazio', () => {
            expect(() => new Pedido_1.Pedido('1', '', [mockItem1], 'pix', 'realizado', dataAtual)).toThrow('Usuário é obrigatório');
        });
        it('deve lançar erro com itens vazios', () => {
            expect(() => new Pedido_1.Pedido('1', 'user123', [], 'pix', 'realizado', dataAtual)).toThrow('O pedido deve ter pelo menos um item');
        });
    });
    describe('Cálculo de Preço', () => {
        it('deve calcular total com múltiplos itens', () => {
            const pedido = new Pedido_1.Pedido('1', 'user123', [mockItem1, mockItem2], 'pix', 'realizado', dataAtual);
    ...
    describe('Confirmar Pedido', () => {
        ...
        it('deve lançar erro ao confirmar pedido não realizado', () => {
            const pedido = new Pedido_1.Pedido('1', 'user123', [mockItem1], 'pix', 'pendente', dataAtual);
    ...
    describe('Cancelar Pedido', () => {
        ...
        it('deve lançar erro ao cancelar pedido enviado', () => {
            const pedido = new Pedido_1.Pedido('1', 'user123', [mockItem1], 'pix', 'enviado', dataAtual);
        ...
        it('deve lançar erro ao cancelar pedido já cancelado', () => {
            const pedido = new Pedido_1.Pedido('1', 'user123', [mockItem1], 'pix', 'cancelado', dataAtual);
            expect(() => pedido.cancelar()).toThrow('Este pedido não pode mais ser cancelado');
        });
    });
});
