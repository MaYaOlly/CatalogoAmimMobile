...
class PedidoService {
     ...
     */
    constructor(pedidoRepository, produtoRepository, usuarioRepository, cupomService) {
    ...
    async criarPedido(dados) {
        ...
        for (const item of dados.itens) {
            ...
            const novoItem = new ItemPedido_1.ItemPedido('', // O ID será gerado pelo banco de dados
            produto, item.quantidade, produto.preco, // Preço no momento da compra
            '' // O ID do pedido será associado pelo repositório
            );
            itensPedido.push(novoItem);
        }
        // 3. Criar a entidade Pedido com os dados validados
        const novoPedido = new Pedido_1.Pedido('', // O ID será gerado pelo banco
        dados.usuarioId, itensPedido, dados.formaPagamento, 'realizado', // Status inicial do pedido
        new Date());
